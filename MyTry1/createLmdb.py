    ...
    with open(descriptFile, 'r') as f:
        ...
        for label in range(n):
            ...
            for pic in range(int(encounter)):
                img = np.array(I.open(os.path.join(dataRoot, name, '%s_%04d.jpg'%(name, pic + 1))).resize(newSize)).swapaxes(0, 2) # Swap to BGR
                objs.append(img)
                labels.append(label)
    return objs, labels
...
if __name__ == '__main__':
    ...
    print('Creating lmdb ...')
    os.mkdir('./lmdb/')
    objs, labels = getImagePackage('./lfw_rules/peopleDevTrain.txt', '/home/share/lfw/' ,(39, 31))
    pushToLMDB(objs, labels, './lmdb/')
    print('\tDone.')
