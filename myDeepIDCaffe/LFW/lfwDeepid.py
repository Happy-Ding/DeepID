...
def solverConstructor(filename, encounter):
    solveParas = dict(train_net='"./lfw_deepid_' + str(encounter) + '.prototxt"', base_lr=0.050, weight_decay=0.001,\
        lr_policy='"inv"', gamma=0.001, power=0.75, display=10, solver_mode="GPU", snapshot=500,\
        snapshot_prefix='"./snapshot_lfw_' + str(encounter) + '/"')
...
def dash(solver, predictLayer, N):
    ...
    for it in range(N):
        ...
        print 'Train: [%04d]: %04d/1000=%.5f%%' % (it, correct, correct/10.0)
        # SGD by Caffe
...
if __name__ == '__main__':
    ...
    for i in range(60):
        print 'Dashing...', i
        try:
            rmtree('./snapshot_lfw_' + str(i) + '/')
        except:
            pass
        os.mkdir('./snapshot_lfw_' + str(i) + '/')
        with open('./lfw_deepid_' + str(i) + '.prototxt', 'w') as f:
            f.write(str(protoConstructor('./lmdb_' + str(i), 100)))
        print ' Net construct ok.'
        mySolver = caffe.SGDSolver(solverConstructor('deepidSolver' + str(i) + '.prototxt', i))
        dash(mySolver, 'fullconnect', 100)
